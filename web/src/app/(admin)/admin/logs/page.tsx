...
type AuditLog = {
  ...
  targetType: string;
  targetId: string;
  /** Enriched server-side for user-targeted events */
  targetEmail?: string;
...
/** Truncate an email to "user@domain" fitting narrow columns without the 4-dots problem. */
function shortEmail(email: string): string {
  if (!email) return "—";
  const atIdx = email.indexOf("@");
  if (atIdx === -1) return email.length > 22 ? `${email.slice(0, 20)}…` : email;
  const local = email.slice(0, atIdx);
  const domain = email.slice(atIdx + 1);
  const shortLocal = local.length > 14 ? `${local.slice(0, 12)}…` : local;
  const shortDomain = domain.length > 18 ? `${domain.slice(0, 16)}…` : domain;
  return `${shortLocal}@${shortDomain}`;
}

function targetLabel(log: AuditLog): string {
  // User targets are enriched server-side with email / displayName
  if (log.targetType === "user") {
    if (log.targetDisplayName) return log.targetDisplayName;
    if (log.targetEmail) return log.targetEmail;
    return `Owner (${log.targetId.slice(0, 8)}…)`;
  }
  // Restaurant / category / menuItem → prefer name from meta
  const name =
    typeof log.meta?.name === "string"
      ? log.meta.name
      : typeof log.meta?.restaurantName === "string"
        ? log.meta.restaurantName
        : null;
  if (name) return name;
  const short = log.targetId.length > 12 ? `${log.targetId.slice(0, 10)}…` : log.targetId;
  return short;
}
...
function MetaRow({ meta }: { meta: Record<string, unknown> }) {
  ...
  if (entries.length === 0)
    return <p className="text-xs text-[#8a8173]">No extra details.</p>;
  return (
    <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1.5 text-xs">
      {entries.map(([k, v]) => (
        <>
          <dt key={`k-${k}`} className="font-medium text-[#5c554a] capitalize">
...
function LogRow({ log }: { log: AuditLog }) {
  ...
  const hasMeta =
    log.meta &&
    Object.values(log.meta).some((v) => v !== undefined && v !== null && v !== "");

  return (
    <>
      <tr
        className={cn(
          "border-b border-[#14110e]/5 transition-colors",
          hasMeta && "cursor-pointer hover:bg-[#14110e]/[0.025]",
        )}
      ...
      >
        ...
        <td className="w-8 py-3 align-top">
          {hasMeta ? (
            <button
              type="button"
              className="rounded p-0.5 text-[#b0a89c] transition-colors hover:text-[#14110e]"
...
export default function LogsPage() {
  ...
  const loadData = useCallback(async (force = false, cursor?: string) => {
  ...
  const filtered = useMemo(() => {
    let result = logs;
    if (group !== "all") {
      result = result.filter((l) => resolveAction(l.action).group === group);
    }
    if (actorSearch.trim()) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex overflow-hidden rounded-lg border border-[#14110e]/10 bg-white shadow-sm">
          {FILTER_TABS.map((tab) => (
            <button
              key={tab.value}
              type="button"
      ...
      {/* Table */}
      <div className="overflow-hidden rounded-xl border border-[#14110e]/8 bg-white shadow-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td
                    colSpan={5}
                    className="py-16 text-center text-sm text-[#8a8173]"
                  >
                    {logs.length === 0
        ...
        {nextCursor && !loading ? (
          <div className="border-t border-[#14110e]/8 px-4 py-3 text-center">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => nextCursor && void loadData(false, nextCursor)}
